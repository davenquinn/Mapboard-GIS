...
laptop handling geospatial operations. It is designed to make
digitally composing geologic maps a quicker and more human-oriented process.

## Pen-based mapping

The most important feature of **Mapboard GIS** is its preference for
pen-based input. "Stream digitizing" is available on other GIS platforms,
and is reasonably effective when combined with accessories such as Wacom graphics tablets.

Tablet-based computing provides new and richer capabilities for pen-based input.
**Mapboard GIS** fully utilizes the precision-capture capabilities of Apple's iPad platform 
to support painless drawing, erasing, and redrafting of linework. This reduces the digital hurdles to making
a well-designed map that faithfully represents interpretations.

## Balancing power and simplicity

Computers are complex devices that can add a lot of unexpected hurdles to field workflows.
This is especially true when combined with capable but dense GIS software.
...
processes. A side benefit of this simplicity, potentially, is easier use of the app in
educational settings. We plan to explore this aspect further in the future.

## Topology-aware

One of the major hurdles to building a fully defined geologic map is *topology*, the
part of the mapping process where boundaries are joined to fill space. In traditional
GIS workflows, this process happens after all linework is added to digitized and finalized.
Even with the quicker drawing tools provided by **Mapboard GIS**, this is a major technical
obstacle, as it prevents seeing the output of your mapping until you have finished entering
the data. Building a process where changes are reflected in topology as soon
as they are entered, will make it much simpler to see the effects of changes in real time.

**Mapboard GIS** supports an innovative iterative mapping workflow. Its topological
solver can solve the boundaries of map polygons in real time, showing the output of the
mapping process in real time. This also reduces the risk that major topological
errors will be propagated, and eliminates the "data cleaning" that has to be done before
